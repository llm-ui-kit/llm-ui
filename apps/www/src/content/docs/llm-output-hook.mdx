...
<PackageInstall client:load packages={["llm-ui"]} />
...
You can pass the full chat response or pass the streamed chat responses as you receive more tokens from the LLM.
...
const block = {
  ...
  lookBack: ({
    ...
    return {
      ...
      output: "<buttons><button>my b</button><buttons>"
      visibleText: "my b" // the visible text the user will actually see
    }
  }
}
```
...
The fallback block is the block to use when no other block matches. This is usually the [markdown block](/docs/markdown-block).

## `throttle` (function) (optional)

Defaults to [`throttleBasic`](/docs/throttle#basic).

The throttle function allows `useLLMOutput` to lag behind the actual LLM output.

Read more about throttle functions in the [throttle docs](/docs/throttle).

## `loopIndex` (number) (optional)

This is only needed when you wish to loop the stream output. For example when using the `useStreamExample` hook.
...
Similar to the `loopIndex` parameter, only needed for looping examples.
